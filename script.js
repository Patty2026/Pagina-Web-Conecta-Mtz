...
function requestUserLocation() {
  ...
  if (!navigator.geolocation) {
    setSelectedLocation(defaultLocation.latitud, defaultLocation.longitud, 'GPS no disponible. Mueve el marcador para indicar la zona');
    return;
  }
...
function markerIconForReport(report) {
  return L.divIcon({ className: 'incident-map-marker', html: `<span>${getCategoryIcon(report.tipo)}</span>`, iconSize: [38, 38], iconAnchor: [19, 38], popupAnchor: [0, -34] });
}
...
function drawReportsOnMap(reports = cachedReports) {
  ...
  validReports.forEach(report => {
    ...
    marker.on('click', () => {
      ...
      updateMapInfoCard(`${folio} · ${tipo}`, `${estado} · ${descripcion}`, getCategoryIcon(tipo));
    });
  });
...
function renderReportCard(report) {
  ...
  return `<div class="report-card" data-report-id="${report.id}"><span class="icon cyan">${getCategoryIcon(tipo)}</span><div><b>${folio} · ${tipo}</b><small>${status} · ${desc}</small></div><span>›</span></div>`;
}
...
function attachReportCardEvents(container) {
  container.querySelectorAll('[data-report-id]').forEach(card => {
    card.addEventListener('click', () => {
      ...
      selectedReport = cachedReports.find(report => report.id === id) || cachedUserReports.find(report => report.id === id) || null;
...
async function loadUserReports() {
  ...
  list.innerHTML = `<div class="report-card"><span class="icon orange">⏳</span><div><b>Cargando reportes...</b><small>Consultando Firestore</small></div><span>·</span></div>`;
  try {
    ...
    if (!cachedUserReports.length) {
      list.innerHTML = `<div class="report-card"><span class="icon green">ℹ️</span><div><b>Sin reportes todavía</b><small>Crea tu primera incidencia</small></div><span>›</span></div>`;
      return;
    }
    ...
    console.error(error);
    list.innerHTML = `<div class="report-card"><span class="icon orange">⚠️</span><div><b>No se pudieron cargar los reportes</b><small>Revisa conexión o reglas de Firestore</small></div><span>·</span></div>`;
  }
}
...
function renderTrackingScreen() {
  ...
  const folio = report.folio || `#${report.id?.slice(0, 8) || 'INC'}`;
  const desc = report.descripcion || 'Sin descripción';
  if (banner) banner.innerHTML = `${status}<br><small>${status === 'Resuelto' ? 'Tu reporte fue marcado como resuelto' : 'Tu reporte está siendo atendido'}</small>`;
  if (title) title.innerHTML = `${folio}<br><span>${tipo}</span><br><small>${desc}</small>`;
  if (timeline) {
    const steps = ['Pendiente', 'En revisión', 'En proceso', 'Resuelto'];
    const currentIndex = steps.indexOf(status);
    timeline.innerHTML = steps.map((step, index) => `<p>${index <= currentIndex ? '✅' : '⚪'} ${step}</p>`).join('');
  }
}
...
function renderProfileReports() {
  ...
  if (!reports.length) {
    container.innerHTML = `<div class="empty-state"><b>No hay reportes disponibles</b><small>Cuando registres una incidencia aparecerá aquí.</small></div>`;
    return;
  }
  container.innerHTML = reports.map(report => {
    ...
    const tipo = report.tipo || 'Reporte ciudadano';
    const desc = report.descripcion || 'Sin descripción';
    return `<div class="profile-report-item" data-profile-report="${report.id}"><span>${getCategoryIcon(tipo)}</span><div><b>${folio}</b><small>${tipo}</small><small>${desc}</small></div><span class="mini-status">${status}</span><div class="profile-actions"><button type="button" data-profile-view="${report.id}">Ver</button><button type="button" data-profile-map="${report.id}">Mapa</button><button type="button" data-profile-delete="${report.id}" class="danger">Eliminar</button></div></div>`;
  }).join('');
  container.querySelectorAll('[data-profile-view]').forEach(button => {
    button.addEventListener('click', () => {
      selectedReport = reports.find(report => report.id === button.dataset.profileView);
      showScreen('trackingScreen');
    });
  });
  container.querySelectorAll('[data-profile-map]').forEach(button => {
    button.addEventListener('click', () => {
      selectedReport = reports.find(report => report.id === button.dataset.profileMap);
      showScreen('mapScreen');
    });
  });
  container.querySelectorAll('[data-profile-delete]').forEach(button => {
    button.addEventListener('click', () => alert('Función demo: para eliminar reportes se requiere habilitar deleteDoc en Firebase.'));
  });
}
...
dots.forEach((dot, index) => dot.addEventListener('click', () => { currentSlide = index; renderSlide(currentSlide); }));
carousel?.addEventListener('touchstart', event => { startX = event.touches[0].clientX; });
...
document.getElementById('sendReport')?.addEventListener('click', async () => {
  ...
  const evidenceFile = document.getElementById('evidenceFile')?.files?.[0] || null;
  try {
    await crearIncidenciaConEvidencia({
      folio,
...
document.getElementById('loginForm')?.addEventListener('submit', async event => {
  ...
  try {
    ...
    localStorage.setItem('conectaPerfil', JSON.stringify({ ...JSON.parse(localStorage.getItem('conectaPerfil') || '{}'), correo: email, nombre: email.split('@')[0], rol: role }));
...
document.getElementById('registerBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    currentUser = credential.user;
    localStorage.setItem('conectaPerfil', JSON.stringify({ correo: email, nombre: email.split('@')[0], telefono: '', rol: role }));
...
escucharSesion(async user => {
  ...
  if (user) {
    ...
    localStorage.setItem('conectaPerfil', JSON.stringify({ ...stored, correo: user.email, nombre: stored.nombre || user.email.split('@')[0] }));
    fillProfileDataForm();
    await loadUserReports();
  }
});
